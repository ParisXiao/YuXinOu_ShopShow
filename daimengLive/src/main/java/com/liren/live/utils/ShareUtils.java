...
public class ShareUtils {
    ...
    public static void share(final Context context, final int index, final SimpleUserInfo user, final PlatformActionListener listener){
        ...
        try {
            ...
            String  shareUrl = configObj.getString("app_android");

            if(index == 1 || index == 2){
                shareUrl = configObj.getString("wx_siteurl") + user.id;
            }

            String[] names = new  String[]{SinaWeibo.NAME,Wechat.NAME,WechatMoments.NAME,QQ.NAME,QZone.NAME};

            share(context,names[index],configObj.getString("share_title")
                    ,user.user_nicename + configObj.getString("share_des"),user,shareUrl,listener);
