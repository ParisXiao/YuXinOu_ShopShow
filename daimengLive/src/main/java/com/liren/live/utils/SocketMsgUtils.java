...
/**
 * Created by liren on 2016/12/26.
 */
...
public class SocketMsgUtils {
    ...
    public int getPushVip(){
        try {
            JSONObject  jsct=new JSONObject(ct);
            if(jsct!=null){
               return  jsct.getInt("vip_type");
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
   ...
   public int getVipType(){
       if (mContentJson != null) {
           try {
               return mContentJson.getInt("viptype");
           } catch (JSONException e) {
               e.printStackTrace();
           }
       }
    ...
    public SocketMsgUtils setViptype(String viptype){
